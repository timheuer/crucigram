...
- **Active-game exclusion**: `PuzzleWarmupService.currentGameWords()` returns the union of solved words from persistence plus any words currently in active daily/unlimited games, ensuring all of those are excluded when warming the next unlimited puzzle.
- **Just-served puzzle exclusion**: When `PuzzleWarmupService.unlimitedPuzzle()` serves a puzzle and immediately triggers the next background warmup, it passes the served puzzle's words as additional exclusions — those words are not yet in persistence (game state not saved) nor in solved words (not completed), so they must be excluded explicitly.
