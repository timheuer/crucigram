...
struct PuzzleGenerationPipelineTests {
  ...
  func wordEntrySourceCodable() throws {
    let entry = WordEntry(
      direction: .across, text: "CRANE", clue: "Tall bird", startRow: 0, startCol: 0, source: .ai)
    let data = try JSONEncoder().encode(entry)
    let decoded = try JSONDecoder().decode(WordEntry.self, from: data)
    #expect(decoded.source == .ai)

    let bundled = WordEntry(
      direction: .down, text: "OAK", clue: "Shade tree", startRow: 1, startCol: 2, source: .bundled)
  ...
  @Test("Fallback with AI clues is preserved even without AI words in grid")
  func fallbackWithAICluesPreservedWhenNoAIWordsPlaced() {
    let resolved = PuzzleGeneratorService.resolveAIGenerationStatus(
      aiWordCount: 0,
      aiGenerationStatus: .fallbackWithAIClues,
      aiGenerationDetail: "Rewrote 24 clues with Apple Intelligence."
    )

    #expect(resolved.0 == .fallbackWithAIClues)
    #expect(resolved.1 == "Rewrote 24 clues with Apple Intelligence.")
  }

  // MARK: - Grid Size Distribution

  @Test(
    "Grid size distribution heavily favors 6x6",
    arguments: [
  ...
  func sixAndSevenMoreCommon() {
    ...
    #expect(
      sixCount > fiveCount,
      "6×6 should appear more often than 5×5 (got \(sixCount) vs \(fiveCount))")
    #expect(
      sevenCount > fiveCount,
      "7×7 should appear more often than 5×5 (got \(sevenCount) vs \(fiveCount))")
  ...
  func wordLengthMix() {
    ...
    #expect(
      lengthCounts.keys.count >= 2, "Should have at least 2 different word lengths across puzzles")
  }
}
