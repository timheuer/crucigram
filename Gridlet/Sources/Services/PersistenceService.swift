...
final class PersistenceService: @unchecked Sendable {
    ...
    private var dailyGameStateURL: URL { documentsURL.appendingPathComponent("daily_game_state.json") }
    private var unlimitedGameStateURL: URL { documentsURL.appendingPathComponent("unlimited_game_state.json") }
    ...
    private init() {
        try? FileManager.default.createDirectory(at: dailyCacheDirectory, withIntermediateDirectories: true)
    }
    ...
    private var unlimitedPuzzleURL: URL { documentsURL.appendingPathComponent("unlimited_puzzle.json") }
    ...
    func pruneOldDailyCache(keepDays: Int = 7) {
        guard let files = try? FileManager.default.contentsOfDirectory(at: dailyCacheDirectory, includingPropertiesForKeys: nil) else { return }
